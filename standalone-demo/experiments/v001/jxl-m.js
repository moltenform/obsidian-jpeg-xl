(function () {
  ...
  async function decode(img, isCSS, isSource) {
    ...
    workers[jxlSrc].addEventListener('message', (m)=>{
          alert('got a message back')
          m.data.imgData && requestAnimationFrame(() => imgDataToDataURL(img, m.data.imgData, isCSS, isSource));
      });
  }
  ...
  async function bengo() {
    ...
    console.log('wwww')
    const img = document.getElementById("thisid")
    alert(img)
    const isCss = false
